...
namespace game_ideas
{
    public class PlayerAttackPassive2 : MonoBehaviour
    {
        ...
        private void CreateDropMissileAttack(Transform playerTransform)
        {
            ...
            // find and display the object by pool name at pooling manager script
            GameObject poolObj = poolingManager.GetPooledObjectsPlayerArm(objectPoolName);
            poolObj.transform.rotation = Quaternion.Euler(playerTransform.rotation.x, playerTransform.rotation.y, 0f);
            poolObj.transform.position = playerTransform.position;
            poolObj.SetActive(true);

            // check if player is descending to increase drop velocity
            if (playerManager.moveDescending)
            {
                poolObj.GetComponent<DropAttack>().playerDescending = true; // set the player descending to increase drop velocity
            }
        }
